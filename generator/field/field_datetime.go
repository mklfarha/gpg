...
func DatetimeFieldTemplate(f entity.Field, e entity.Entity) Template {
	...
	graphGenToMapper := fmt.Sprintf("i.%s.Format(\"2006-01-02 15:04:05\")", helpers.ToCamelCase(f.Identifier))
	graphGenFromMapper := fmt.Sprintf("ParseTime(i.%s)", helpers.ToCamelCase(f.Identifier))
	graphGenFromMapperOptional := fmt.Sprintf("ParseTimeFromPointer(i.%s)", helpers.ToCamelCase(f.Identifier))
	if !f.Required || generatedDate {
		graphGenToMapper = fmt.Sprintf("FormatTimeToPointer(i.%s)", helpers.ToCamelCase(f.Identifier))
